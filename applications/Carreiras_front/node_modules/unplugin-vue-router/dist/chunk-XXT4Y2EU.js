...
var _chunkESDZCTQEjs = require('./chunk-ESDZCTQE.js');
...
var _TreeNodeValueBase = (_class = class {
  ...
  get path() {
    ...
    return (!parentPath || parentPath === "/") && pathSegment === "" ? "/" : _chunkESDZCTQEjs.joinPath.call(void 0, parentPath || "", pathSegment);
  }
  toString() {
  ...
  get overrides() {
    return [...this._overrides.entries()].sort(
      ...
      return _chunkESDZCTQEjs.mergeRouteRecordOverride.call(void 0, acc, routeBlock);
    }, {});
  }
  ...
  mergeOverride(filePath, routeBlock) {
    ...
    this._overrides.set(
      filePath,
      _chunkESDZCTQEjs.mergeRouteRecordOverride.call(void 0, existing, routeBlock)
    );
  }
  /**
...
var TreeNode = (_class4 = class _TreeNode {
  ...
  insert(path, filePath = path) {
    ...
    const { tail, segment, viewName, isComponent } = splitFilePath(
      ...
      _chunkESDZCTQEjs.resolveOverridableOption.call(void 0, 
        this.options.extensions,
        _optionalChain([folderOptions, 'optionalAccess', _4 => _4.extensions])
      )
    );
  ...
  remove(path) {
    ...
    const { tail, segment, viewName, isComponent } = splitFilePath(
      ...
      _chunkESDZCTQEjs.resolveOverridableOption.call(void 0, 
        this.options.extensions,
        _optionalChain([folderOptions, 'optionalAccess', _5 => _5.extensions])
      )
    );
    const child = this.children.get(segment);
    if (!child) {
   ...
   */
  get path() {
    return _nullishCoalesce(this.value.overrides.path, () => ( (_optionalChain([this, 'access', _6 => _6.parent, 'optionalAccess', _7 => _7.isRoot, 'call', _8 => _8()]) ? "/" : "") + this.value.pathSegment));
  }
  /**
...
function splitFilePath(filePath, extensions) {
  ...
  let segment = head;
  if (!tail) {
    segment = _chunkESDZCTQEjs.trimExtension.call(void 0, head, extensions);
  }
...
function findFolderOptions(folderOptions, filePath) {
...
async function getRouteBlock(path, options) {
  ...
  const blockStr = _optionalChain([parsedSFC, 'optionalAccess', _9 => _9.customBlocks, 'access', _10 => _10.find, 'call', _11 => _11((b) => b.type === "route")]);
  if (!blockStr)
    return;
  let result = parseCustomBlock(blockStr, path, options);
  if (result) {
    if (result.path != null && !result.path.startsWith("/")) {
      _chunkESDZCTQEjs.warn.call(void 0, `Overridden path must start with "/". Found in "${path}".`);
    }
  }
  return result;
}
function parseCustomBlock(block, filePath, options) {
  ...
  if (lang === "json5") {
    try {
      ...
      _chunkESDZCTQEjs.warn.call(void 0, 
...
var RoutesFolderWatcher = class {
  ...
  on(event, handler) {
    this.watcher.on(event, (filePath) => {
      ...
      handler({
        filePath,
        routePath: _chunkESDZCTQEjs.asRoutePath.call(void 0, { src: this.src, path: this.path }, filePath)
      });
    });
    return this;
  }
...
function resolveFolderOptions(globalOptions, folderOptions) {
  ...
  return {
    ...
    pattern: _chunkESDZCTQEjs.appendExtensionListToPattern.call(void 0, 
      filePatterns,
...
function extractDefinePageNameAndPath(sfcCode, id) {
  ...
  const setupAst = getSetupAst();
  const definePageNodes = (_nullishCoalesce(_optionalChain([setupAst, 'optionalAccess', _14 => _14.body]), () => ( []))).map((node) => {
    if (node.type === "ExpressionStatement")
      node = node.expression;
    return _common.isCallOf.call(void 0, node, MACRO_DEFINE_PAGE) ? node : null;
  }).filter((node) => !!node);
    ...
    if (prop.type === "ObjectProperty" && prop.key.type === "Identifier") {
      if (prop.key.name === "name") {
        if (prop.value.type !== "StringLiteral") {
          _chunkESDZCTQEjs.warn.call(void 0, `route name must be a string literal. Found in "${id}".`);
        } else {
          routeInfo.name = prop.value.value;
        }
      } else if (prop.key.name === "path") {
        if (prop.value.type !== "StringLiteral") {
          _chunkESDZCTQEjs.warn.call(void 0, `route path must be a string literal. Found in "${id}".`);
        } else {
...
function createRoutesContext(options) {
  ...
  async function scanPages(startWatchers = true) {
        ...
        return _fastglob2.default.call(void 0, folder.pattern, {
          ...
          (files) => Promise.all(
            files.map((file) => _pathe.resolve.call(void 0, folder.src, file)).map(
              (file) => addPage({
                routePath: _chunkESDZCTQEjs.asRoutePath.call(void 0, folder, file),
                filePath: file
              })
            )
          )
    ...
    for (const route of editableRoutes) {
      await _optionalChain([options, 'access', _15 => _15.extendRoute, 'optionalCall', _16 => _16(route)]);
    }
  ...
  function generateRoutes() {
    const importsMap = new (0, _chunkESDZCTQEjs.ImportsMap)();
...
`;
  ...
  async function _writeConfigFiles() {
    ...
    _chunkESDZCTQEjs.logTree.call(void 0, routeTree, logger.log);
    if (dts) {
      ...
      if (lastDTS !== content) {
        ...
        lastDTS = content;
        _optionalChain([server, 'optionalAccess', _21 => _21.invalidate, 'call', _22 => _22(MODULE_ROUTES_PATH)]);
        _optionalChain([server, 'optionalAccess', _23 => _23.invalidate, 'call', _24 => _24(MODULE_VUE_ROUTER)]);
        _optionalChain([server, 'optionalAccess', _25 => _25.reload, 'call', _26 => _26()]);
      }
    }
    logger.timeEnd("writeConfigFiles");
  }
  const writeConfigFiles = _chunkESDZCTQEjs.throttle.call(void 0, _writeConfigFiles, 500, 100);
...
var src_default = _unplugin.createUnplugin.call(void 0, (opt = {}, _meta) => {
  const options = _chunkESDZCTQEjs.resolveOptions.call(void 0, opt);
  ...
  const pageFilePattern = _chunkESDZCTQEjs.appendExtensionListToPattern.call(void 0, 
    options.filePatterns,
    _chunkESDZCTQEjs.mergeAllExtensions.call(void 0, options)
  );
